...
data:
  ...
  bundledCode: "#line 1 \"verify/yosupo_library_checker/data_structure/Static_Range_Frequency.test.cpp\"\
    ...
    \ vector<int>& uniques, int x) {\n    auto it = lower_bound(uniques.begin(), uniques.end(),\
    \ x);\n    if (it == uniques.end() || *it != x) return -1;\n    return distance(uniques.begin(),\
    \ it);\n}\n\nvector<int> verify() {\n    int N, Q; cin >> N >> Q;\n    vector<int>\
    \ a(N);\n    for (int i = 0; i < N; ++i) scanf(\"%d\", &a[i]);\n\n    auto uniques\
    \ = get_uniques(a);\n\n    Persistent_Segment_Tree<int> S(vector<int>(uniques.size(),\
    \ 0), add<int>, 0);\n    for (int i = 0; i < N; ++i) {\n        int val_idx =\
    \ get_index(uniques, a[i]);\n        S.increment();\n        S.update(val_idx,\
    \ S.get(i, val_idx) + 1);\n    }\n\n    vector<int> ans(Q);\n    for (int q =\
  ...
  verifiedWith: []
documentation_of: verify/yosupo_library_checker/data_structure/Static_Range_Frequency.test.cpp
layout: document
redirect_from:
- /verify/verify/yosupo_library_checker/data_structure/Static_Range_Frequency.test.cpp
- /verify/verify/yosupo_library_checker/data_structure/Static_Range_Frequency.test.cpp.html
