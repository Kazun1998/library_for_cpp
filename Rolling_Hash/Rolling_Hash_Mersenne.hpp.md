...
data:
  ...
  attributes:
    document_title: "mod (2^61 - 1) \u4E0A\u3067\u8A08\u7B97\u3059\u308B\u30ED\u30FC\
      \u30EA\u30F3\u30B0\u30CF\u30C3\u30B7\u30E5"
    links: []
  bundledCode: "#line 2 \"Rolling_Hash/Rolling_Hash_Mersenne.hpp\"\n\n#line 2 \"template/template.hpp\"\
    \n\nusing namespace std;\n\n// intrinstic\n#include <immintrin.h>\n\n#include\
    \ <algorithm>\n#include <array>\n#include <bitset>\n#include <cassert>\n#include\
    \ <cctype>\n#include <cfenv>\n#include <cfloat>\n#include <chrono>\n#include <cinttypes>\n\
    #include <climits>\n#include <cmath>\n#include <complex>\n#include <concepts>\n\
    ...
    \   };\n} // namespace rolling_hash\n#line 5 \"Rolling_Hash/Rolling_Hash_Mersenne.hpp\"\
    \n\nnamespace rolling_hash {\n    /// @brief mod (2^61 - 1) \u4E0A\u3067\u8A08\
    \u7B97\u3059\u308B\u30ED\u30FC\u30EA\u30F3\u30B0\u30CF\u30C3\u30B7\u30E5\n   \
    \ class Rolling_Hash_Mersenne : public Rolling_Hash_Base<unsigned long long, Rolling_Hash_Mersenne>\
    \ {\n        using ull = unsigned long long;\n        using Base = Rolling_Hash_Base<ull,\
    \ Rolling_Hash_Mersenne>;\n        friend Base;\n\n        static constexpr ull\
    ...
    \ - (b - a));\n        }\n\n        template<typename T>\n        static vector<ull>\
    \ to_vector(const vector<T> &X) {\n            vector<ull> data(X.size());\n \
    \           for (int i = 0; i < (int)X.size(); ++i) data[i] = (ull)X[i];\n   \
    \         return data;\n        }\n\n        static vector<ull> to_vector(const\
    \ string &S) {\n            vector<ull> data(S.size());\n            for (int\
    \ i = 0; i < (int)S.size(); ++i) data[i] = (ull)(unsigned char)S[i];\n       \
    \     return data;\n        }\n\n        public:\n        using Base::count;\n\
    \        using Base::find;\n        using Base::rfind;\n\n        template<typename\
    \ T>\n        Rolling_Hash_Mersenne(const vector<T> &X, const ull base): Base(base)\
    \ { this->build(to_vector(X)); }\n\n        template<typename T>\n        int\
    \ count(const vector<T> &X, int start) const { return count(to_vector(X), start);\
    \ }\n        template<typename T>\n        int count(const vector<T> &X) const\
    \ { return count(to_vector(X), 0); }\n\n        template<typename T>\n       \
    \ int find(const vector<T> &X, int start) const { return find(to_vector(X), start);\
    \ }\n        template<typename T>\n        int find(const vector<T> &X) const\
    \ { return find(to_vector(X), 0); }\n\n        template<typename T>\n        int\
    \ rfind(const vector<T> &X, int pos) const { return rfind(to_vector(X), pos);\
    \ }\n        template<typename T>\n        int rfind(const vector<T> &X) const\
    \ { return rfind(to_vector(X), size()); }\n\n        // \u6587\u5B57\u5217\u7528\
    \u30AA\u30FC\u30D0\u30FC\u30ED\u30FC\u30C9\n        Rolling_Hash_Mersenne(const\
    \ string &S, const ull base): Rolling_Hash_Mersenne(to_vector(S), base) {}\n\n\
...
|日付|内容|
|:---:|:---:|
|2026/08/09| Rolling_Hash_Mersenne 実装 |
