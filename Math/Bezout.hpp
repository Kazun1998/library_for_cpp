...
namespace bezout {
    ...
    template<integral T>
    tuple<T, T, T, T, T, T> Solve(T a, T b, T c, const T lx, const T rx, const T ly, const T ry, const pair<T, T> &hint) {
        T p, q;
        tie (p, q) = hint;
        const T g = a * p + b * q;
        ...
        if (kl > kr) throw NotExist();

        return { c * p, b, c * q, -a, kl, kr };
    };

    template<integral T>
    tuple<T, T, T, T, T, T> Solve(T a, T b, T c, const T lx, const T rx, const T ly, const T ry) {
        auto [p, q, g] = Extended_Euclid<T>(a, b);
